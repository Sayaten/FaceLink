...
public class MatrixSolver {
    ...
    public MatrixSolver(int[][] mask, ArrayList<Coord> selectionArea, 
    					Bitmap image, Bitmap selectedImage,
    	...
    	for (int i = 0; i < N; i++) {
    		...
    		int RGB = selectedImage.getPixel(selX, selY);
    		...
    		for (int k = 0; k < dP.length; k++) {
    			...
    			if (index == -1) { //It's a border pixel
    				...
    				selX = x2 - xMin;
    				selY = y2 - yMin;
    				RGB = selectedImage.getPixel(selX, selY);
    ...
    public void updateImage(Bitmap selectedImage) {
    	for (int i = 0; i < X.length; i++) {
    		...
    		selectedImage.setPixel(x, y, RGB);
    	}
    }
}